function msfun_arduino_gyroread(block)
...
%%
    function CheckPrms(block)        
        try
            ...
            validateattributes(block.DialogPrm(2).Data, {'char'}, {'nonempty'}); % axis
...
%%
    function Output(block)
        % fprintf('%s: Output\n', getfullname(block.BlockHandle));
        gyroValue = myArduino.gyroRead(axis);
        if isempty(gyroValue)
            gyroValue = 0;
        end        
        block.OutputPort(1).Data = gyroValue;
    end
