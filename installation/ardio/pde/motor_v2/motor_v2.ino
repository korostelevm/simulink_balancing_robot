...
Adafruit_StepperMotor *stm1 = AFMS.getStepper(200, 1);
Adafruit_StepperMotor *stm2 = AFMS.getStepper(200, 2);
Adafruit_DCMotor *dcm1 = AFMS.getMotor(1);
Adafruit_DCMotor *dcm2 = AFMS.getMotor(2);
Adafruit_DCMotor *dcm3 = AFMS.getMotor(3);
Adafruit_DCMotor *dcm4 = AFMS.getMotor(4);
...
void setup() {
  ...
  /* initialize AF Motor Shield with frequency of 1.6KHz     */
  AFMS.begin();
...
void loop() {
  ...
  if (Serial.available() >0) {
    ...
    switch (s) {
	  ...
	  /* the first received value indicates the mode       
           ...
           s=100 is to read i2c from arduino (two pins)
      ...
      if ((s>100 && s<170) || (s>250 && s!=340 && s!=400)) {
        s=-1;
      }
 ...
 // delay(1500); //wait for the sensor to be ready 
      ...
      if (val>96 && val <100) {
          int axis  = val - 97;
        /* read i2c functin gyro */
        int rho = getGyroValues(axis);
      // digitalWrite(13,LOW);
        Serial.println(rho);
      }
      s=-1;  /* we are done with this so next state is -1    */
...
/* gyro functions below */



int getGyroValues(int axis){
  int val = 0;
  
  if (axis == 0){
  byte xMSB = readRegister(L3G4200D_Address, 0x29);
  byte xLSB = readRegister(L3G4200D_Address, 0x28);
  val = ((xMSB << 8) | xLSB); // x axis
  }
  if (axis == 1){
  byte yMSB = readRegister(L3G4200D_Address, 0x2B);
  byte yLSB = readRegister(L3G4200D_Address, 0x2A);
  val = ((yMSB << 8) | yLSB); //y axys
  }
  if (axis == 2){
